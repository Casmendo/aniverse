...
export default function FavoritesPage() {
  ...
  return (
    <div className="min-h-screen pb-28" style={{ background: 'radial-gradient(ellipse at top, #f8fafc 0%, #ffffff 40%)' }}>
      {/* Header */}
      <div className="sticky top-0 z-40 px-5 h-14 flex items-center bg-white/95 backdrop-blur-xl border-b border-blue-200">
      ...
      <div className="px-4 pt-6">
        {bookmarks.length === 0 ? (
          <div className="text-center py-24 text-slate-600">
            ...
            <Link href="/manga/discover" className="inline-block mt-6 px-6 py-2.5 rounded-full bg-blue-600 text-white text-sm font-bold hover:bg-blue-500 transition-colors">
              Discover Manga
            </Link>
          </div>
        ) : (
              ...
              >
                <div className="relative group">
                  <Link href={`/manga/${bm.mangaId}`} className="block">
                    <div className="relative overflow-hidden rounded-xl border border-blue-200 bg-white group-hover:border-blue-400 transition-all group-hover:-translate-y-1">
                      ...
                      <div className="p-2.5">
                        <h3 className="text-xs font-bold text-slate-700 line-clamp-2 leading-snug">{bm.title}</h3>
                  ...
                  <button
                    ...
                    className="absolute top-2 right-2 w-7 h-7 rounded-full bg-blue-600 flex items-center justify-center text-slate-900 opacity-0 group-hover:opacity-100 transition-opacity shadow-lg"
                  >
            ...
            <div className="flex flex-col gap-2">
              {progress.slice(0, 10).map(p => (
                <Link key={p.mangaId} href={`/manga/read/${p.mangaId}/${p.chapterId}`}
                  className="flex items-center gap-3 p-3 rounded-xl bg-blue-50 border border-blue-200 hover:border-blue-400 transition-all group">
                  <div className="w-12 h-16 rounded-lg overflow-hidden shrink-0 bg-blue-50">
                    {p.coverArt
                      ...
                      : <div className="w-full h-full flex items-center justify-center"><BookOpen size={14} className="text-blue-800" /></div>
                    }
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="text-sm font-bold text-slate-800 group-hover:text-slate-900 line-clamp-1">{p.mangaTitle}</h3>
                    <p className="text-xs text-blue-500/60 font-bold mt-0.5">Chapter {p.chapterNum}</p>
