...
export default function MangaLibraryPage() {
  ...
  return (
    <div className="min-h-screen pb-28" style={{ background: 'radial-gradient(ellipse at top, #f8fafc 0%, #ffffff 40%)' }}>
      {/* Header */}
      <div className="sticky top-0 z-40 px-5 h-14 flex items-center justify-between bg-white/95 backdrop-blur-xl border-b border-blue-200">
        ...
        {progress.length === 0 ? (
          ...
          <div className="flex flex-col gap-3">
            {progress.map((p, i) => (
              ...
              >
                <Link href={`/manga/${p.mangaId}`} className="w-12 h-16 rounded-lg overflow-hidden shrink-0 bg-blue-50">
                  {p.coverArt
                ...
                <div className="flex-1 min-w-0">
                  <Link href={`/manga/${p.mangaId}`}>
                    <h3 className="text-sm font-bold text-slate-800 group-hover:text-slate-900 line-clamp-1 transition-colors">{p.mangaTitle}</h3>
                  </Link>
                  <p className="text-xs text-blue-500/60 font-bold mt-0.5">Ch {p.chapterNum} · Page {p.page}/{p.totalPages}</p>
                ...
                <div className="flex flex-col gap-2 shrink-0">
                  <Link href={`/manga/read/${p.mangaId}/${p.chapterId}`}
                    className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-blue-600 text-white text-[10px] font-bold hover:bg-blue-500 transition-colors">
                    Resume <ChevronRight size={12} />
                  </Link>
                  <button onClick={() => removeProgress(p.mangaId)}
                    className="p-1.5 rounded-lg text-slate-600 hover:text-blue-600 hover:bg-blue-50 transition-all flex items-center justify-center">
                    <Trash2 size={14} />
                  </button>
                </div>
              </motion.div>
