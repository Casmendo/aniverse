    ...
    >
      ...
      <AnimatePresence>
        {(showCtrl || (indicator && indicator.type === 'brightness')) && !isLocked && !buffering && !errMsg && !streamFetchError && (
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }}
            className="absolute left-5 top-[43%] -translate-y-1/2 z-30 pointer-events-none flex flex-col items-center justify-center bg-black/60 backdrop-blur-md w-12 h-44 rounded-2xl shadow-2xl border border-white/10">
      ...
      <AnimatePresence>
        {(showCtrl || (indicator && indicator.type === 'volume')) && !isLocked && !buffering && !errMsg && !streamFetchError && (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.2 }}
            className="absolute right-5 top-[43%] -translate-y-1/2 z-30 pointer-events-none flex flex-col items-center justify-center bg-black/60 backdrop-blur-md w-12 h-44 rounded-2xl shadow-2xl border border-white/10">
