    ...
    >
      ...
      <AnimatePresence>
        {(showCtrl || (indicator && indicator.type === 'brightness')) && !isLocked && !buffering && !errMsg && !streamFetchError && (
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }}
            className="absolute left-5 top-[41%] -translate-y-1/2 z-30 pointer-events-none flex flex-col items-center justify-center bg-black/60 backdrop-blur-md w-12 h-44 rounded-2xl shadow-2xl border border-white/10">
            <Sun size={20} className="text-white mb-2.5" />
            <div className="h-24 w-[6px] bg-white/20 rounded-full mb-2.5 flex flex-col justify-end overflow-hidden">
      ...
      <AnimatePresence>
        {(showCtrl || (indicator && indicator.type === 'volume')) && !isLocked && !buffering && !errMsg && !streamFetchError && (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} transition={{ duration: 0.2 }}
            className="absolute right-5 top-[41%] -translate-y-1/2 z-30 pointer-events-none flex flex-col items-center justify-center bg-black/60 backdrop-blur-md w-12 h-44 rounded-2xl shadow-2xl border border-white/10">
            {muted || volume === 0 ? <VolumeX size={20} className="text-white mb-2.5" /> : <Volume2 size={20} className="text-white mb-2.5" />}
            <div className="h-24 w-[6px] bg-white/20 rounded-full mb-2.5 flex flex-col justify-end overflow-hidden">
